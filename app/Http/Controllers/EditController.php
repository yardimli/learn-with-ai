...
use Illuminate\Support\Facades\Auth; // For transactions
...
PROMPT;
	 ...
	 * @param string $lessonContentTitle Title of the lesson content.
	 ...
	 */
	public static function generateQuestionsForLessonDifficulty(string $llm, string $lessonPrompt, string $lessonContentTitle, string $lessonContentText, string $difficulty, array $existingQuestionTexts, int $maxRetries = 1): array
	{
		$userContent = "Initial Lesson Prompt: " . $lessonPrompt . "\n\n";
		$userContent .= "Current Lesson Title: " . $lessonContentTitle . "\n\n";
		$userContent .= "Current Lesson Text: " . $lessonContentText . "\n\n";
		...
		Log::info("Requesting {$difficulty} question generation for lesson '{$lessonContentTitle}' using LLM: {$llm}");
	...
	public function edit(Lesson $lesson)
	{
		...
		$lesson->lesson_content = is_array($lessonContent) ? $lessonContent : ['title' => null, 'text' => null, 'image_prompt_idea' => null, 'sentences' => []];
	...
	public function generateQuestionBatchAjax(Request $request, Lesson $lesson, string $difficulty)
	{
		...
		$contentTitle = $lessonContent['title'] ?? $lesson->user_title ?? 'Lesson Content';
		...
		if (isset($questionResult['error'])) {
			...
			Log::error($logMsg, ['lesson' => $lesson->id, 'llm' => $llm, 'content_title' => $contentTitle]);
		...
		if (!self::isValidQuestionListResponse($questionResult['questions'])) {
			$errorMsg = "LLM returned an invalid {$difficulty} question structure for lesson '{$contentTitle}'.";
			Log::error($errorMsg, ['lesson' => $lesson->id, 'llm' => $llm, 'content_title' => $contentTitle, 'response' => $questionResult]);
	...
	public function updateLessonContentAjax(Request $request, Lesson $lesson)
	{
		...
		$validator = Validator::make($request->all(), [
			...
			'lesson_text' => 'required|string|min:10|max:2000',
		]);
		...
		try {
				...
				foreach ($oldSentences as $oldSentence) {
					...
					if (!empty($oldSentence['generated_image_id'])) {
						...
						if ($oldImage && in_array($oldImage->source, ['upload', 'freepik'])) {
							try { $oldImage->deleteStorageFiles(); $oldImage->delete(); } catch (Exception $e) { Log::warning("Could not delete old sentence image: " . $e->getMessage()); }
						}
					}
				}
			...
			return response()->json([
				...
				'updated_content' => [ // Send back updated data for JS
					...
					'sentences_cleared' => ($oldText !== $newText)
				]
			]);
		} catch (Exception $e) {
			DB::rollBack();
	...
	public function addYoutubeVideoAjax(Request $request, Lesson $lesson)
	{
		...
		try {
			$response = Http::withHeaders([
				...
				->get("https://{$rapidApiHost}/v2/video/details", ['videoId' => $youtubeVideoId]);

			if (!$response->successful()) {
				Log::error("RapidAPI request failed for video {$youtubeVideoId}.", ['status' => $response->status(), 'body' => $response->body()]);
				throw new Exception("Failed to fetch video details from API (Status: {$response->status()}).");
			}
			$videoData = $response->json();
			if (!$videoData || !($videoData['status'] ?? false)) {
				Log::error("RapidAPI returned error or invalid data for video {$youtubeVideoId}.", ['response' => $videoData]);
			...
			if (isset($videoData['subtitles']['items']) && is_array($videoData['subtitles']['items'])) {
				...
				if ($subtitleUrl !== '') {
					try {
						$subtitleResponse = Http::timeout(20)->get($subtitle['url']); // Changed from $subtitle['url'] to $subtitleUrl
						if ($subtitleResponse->successful()) {
							...
							if ($xml !== false) {
								foreach ($xml->text as $textNode) {
									...
									// Log::info("Subtitle text: " . (string)$textNode); // Reduced logging
								}
								$plaintext = html_entity_decode($subtitleText, ENT_QUOTES | ENT_HTML5);
								$plaintext = preg_replace('/\s+/', ' ', $plaintext);
								$plaintext = trim($plaintext);
							} else {
								Log::info("Failed to parse subtitle XML for track."); // Removed $subtitle['text'] as it might not be defined here
							}
						} else {
							Log::warning("Failed to download subtitle track for {$youtubeVideoId}. Status: " . $subtitleResponse->status());
						}
					} catch (Exception $subE) {
						Log::warning("Error processing subtitle track for {$youtubeVideoId}: " . $subE->getMessage());
					}
				} else {
